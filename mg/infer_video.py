...
def generator_tensor_dict(image_path, mask_path, args):
    ...
    # 临时测试
    # cv2.imwrite('mask_ori.png',mask_ori)
    # mask = (mask_ori==38)
    # print(np.max(mask))
...
if __name__ == '__main__':
    ...
    # inference
    model = model.eval()
    for clip in sorted(os.listdir(os.path.join(args.data_root))):
        # print(dataset, clip)
        clip_path = os.path.join(args.data_root, clip)
        imagesdir_path = os.path.join(args.data_root, clip,args.image_dirname)
        masksdir_path = os.path.join(args.data_root, clip,args.mask_dirname)

        if os.path.isdir(clip_path) and os.path.isdir(imagesdir_path) and os.path.isdir(masksdir_path):
            pass
        else:
            continue
        outdir_path = os.path.join(args.data_root, clip,args.out_dirname)
        if os.path.exists(outdir_path):
            shutil.rmtree(outdir_path)
        utils.make_dir(outdir_path)

        for image_name in os.listdir(imagesdir_path):
            # assume image and mask have the same file name
            image_path = os.path.join(imagesdir_path, image_name)
            mask_path = os.path.join(masksdir_path, image_name.replace(args.image_ext, args.mask_ext))
            ...
            cv2.imwrite(os.path.join(outdir_path, image_name), alpha_pred)
