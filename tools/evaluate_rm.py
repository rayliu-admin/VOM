...
class Evaluator:
    def __init__(self,  # images, gts, 
        data_root,
        pred_name='result', true_name='alphas',trimap_name='trimaps',
        num_workers=16,
        is_eval_fgr=False,
        is_fix_fgr=False,
        is_trimap_wise=True
    ...
    def evaluate(self):
        ...
        with ThreadPoolExecutor(max_workers=self.num_workers) as executor:
            for clip in sorted(os.listdir(os.path.join(self.data_root))):
                # print(dataset, clip)
                if os.path.isdir(os.path.join(self.data_root, clip)):
                    future = executor.submit(self.evaluate_worker, clip, position)
                    tasks.append((clip, future))
                    position += 1
                    
        self.results = [(clip, future.result()) for clip, future in tasks]
        
    def write_excel(self):
        write_path = os.path.join(self.data_root, f'{os.path.basename(self.pred_name)}.xlsx')
        if os.path.exists(write_path):
            os.remove(write_path)
        workbook = xlsxwriter.Workbook(write_path)
        print(os.path.join(self.data_root, f'{os.path.basename(self.pred_name)}.xlsx'))
        ...
        max_columns = 0
        for row, (clip, metrics) in enumerate(self.results):
            ...
            for metricsheet, metric in zip(metricsheets, metrics.values()):
                ...
                metricsheet.write(row + 2, 0, clip)
    ...
    def evaluate_worker(self, clip, position):
        framenames = sorted(os.listdir(os.path.join(self.data_root, clip, self.true_name)))
        framenames_pred = sorted(os.listdir(os.path.join(self.data_root, clip, self.pred_name)))
        ...
        for i, framename in enumerate(tqdm(framenames, desc=f'{clip}', position=position, dynamic_ncols=True)):
            # f'{FG}-{BG}' -> f'{FG}
            # true_clip = clip.split('-')[0] if self.is_fix_fgr else clip
            # print(os.path.join(self.true_dir, dataset, true_clip, 'pha', framename))
            # raise
            try:
                true_pha = cv2.imread(os.path.join(self.data_root, clip, self.true_name, framename), cv2.IMREAD_GRAYSCALE).astype(np.float32) / 255
                pred_pha = cv2.imread(os.path.join(self.data_root, clip, self.pred_name, framename), cv2.IMREAD_GRAYSCALE).astype(np.float32) / 255
                assert np.array_equal(pred_pha.shape, true_pha.shape), f"{pred_pha.shape}, {true_pha.shape}, {self.pred_dir}"
                if self.is_trimap_wise:
                    trimap = cv2.imread(os.path.join(self.data_root, clip, self.trimap_name, framename), cv2.IMREAD_GRAYSCALE)
                ...
                print(self.data_root, clip, self.true_name, framename, 'not found')
                raise
            ...
            if 'fgr_mse' in self.metrics or 'fgr_mad' in self.metrics:
                try:
                    true_fgr = cv2.imread(os.path.join(self.data_root, clip, self.fgr_name, framename), cv2.IMREAD_COLOR).astype(np.float32) / 255
                    pred_fgr = cv2.imread(os.path.join(self.data_root, clip, self.pred_fgr_name, framename), cv2.IMREAD_COLOR).astype(np.float32) / 255
                except:
                    print(self.data_root, clip, self.fgr_name, framename, 'not found')
                    raise
                true_msk = true_pha > 0
...
if __name__ == '__main__':
    import sys
    data_root = '/home/liurui/DATA/realhuman_reformat'

    Evaluator(data_root,
        pred_name='ftpvm-result', true_name='alphas',
        is_trimap_wise=False,
    )